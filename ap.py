...
with st.expander("🧠 Tips Pertanian Harian Otomatis"):
    for _, row in df.iterrows():
        ...
        if row["Suhu Maks (°C)"] > 33:
            tips.append("🔥 Waspadai stres panas")
        if row["Kelembapan (%)"] > 85:
            tips.append("🦠 Potensi jamur dan hama")
        if not tips:
            tips.append("🌿 Cuaca baik untuk menanam atau memanen")

        st.markdown(f"<div style='background-color:#eef9f2;padding:10px;border-radius:10px;margin-bottom:8px;'>"
                    f"<strong>📅 {row['Tanggal'].date()}:</strong> {' | '.join(tips)}</div>", unsafe_allow_html=True)
